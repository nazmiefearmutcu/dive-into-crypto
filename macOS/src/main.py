...
def _as_runtime_file(value: str | Path | None, *, fallback: Path, default_filename: str) -> Path:
    """Resolve a runtime artifact path, treating directory config values as folders."""
    if value is None:
        return fallback
    try:
        text = str(value).strip()
...
def _resolve_pid_file(config: dict[str, Any] | None = None) -> Path:
    """Resolve PID path from explicit pid_path or dashboard_status_path."""
    ...
    try:
        ...
        if not isinstance(dashboard_status_path, (str, Path)):
            return _default_pid_file()
        text = str(dashboard_status_path).strip()
        if text in {"", "."}:
            return _default_pid_file()
        runtime_dir = _as_project_path(dashboard_status_path)
        if runtime_dir == Path("."):
            return _default_pid_file()
        if runtime_dir.suffix:
            runtime_dir = runtime_dir.parent
        return runtime_dir / "bot.pid"
    except Exception:
        return _default_pid_file()
...
def _resolve_log_path(config: dict[str, Any] | None = None) -> str:
    ...
    if not log_path:
        return str(_default_log_file())
    return str(_as_runtime_file(log_path, fallback=_default_log_file(), default_filename="bot.log"))
