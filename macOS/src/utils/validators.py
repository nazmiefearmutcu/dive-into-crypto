...
def validate_config(config: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(polling_interval_seconds, int) or isinstance(polling_interval_seconds, bool) or not isfinite(polling_interval_seconds) or polling_interval_seconds <= 0:
        errors.append("polling_interval_seconds must be a positive integer")

    candle_limit = config.get("candle_limit", 200)
    if not isinstance(candle_limit, int) or isinstance(candle_limit, bool) or not isfinite(candle_limit) or candle_limit <= 0:
    ...
    if not isinstance(max_open_positions, int) or isinstance(max_open_positions, bool) or not isfinite(max_open_positions) or max_open_positions < 0:
