...
class BotService:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.config_watcher = ConfigWatcher(str(config_path))
    ...
    def _cycle(self) -> None:
        ...
        # 10. Legacy dashboard close-command file bridge (no longer active).
        # The command-queue path is authoritative for manual close now.
    ...
    # new source of truth. This method now only cleans stale legacy files.

    def _process_close_commands(self) -> None:
        """Clear stale legacy close command files without executing actions.

        The queue-based path is now the single source of truth for manual close
        commands. This cleanup prevents old or manually-created `close_cmd_*.json`
        artifacts from triggering legacy side effects.
        """
            ...
            try:
                ...
                for tf in self._multi_tfs:
                    try:
                        ...
                        md = MarketDataProvider(self.binance_client, tf_config)
    ...
    def _bind_runtime_artifacts(self, config: dict[str, Any]) -> None:
        """Rebind runtime-backed collaborators from config."""
        runtime_dir = _resolve_runtime_dir(config)
